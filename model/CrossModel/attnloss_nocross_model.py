...
class JointModule(nn.Module):
    ...
    def forward(self, review_input, reply_input, table_input, review_input_mask, reply_input_mask, review_seq_lens, reply_seq_lens):
        ...
        _, _, attn_review, attn_reply = self.attn_layer(grid_feature, review_lstm_embedding, reply_lstm_embedding, review_input_mask, reply_input_mask)
        return grid_feature, review_lstm_embedding, reply_lstm_embedding, attn_review, attn_reply

class AttnLossNoCrossModel(nn.Module):
    
    def __init__(self, args, bertModel):
        super(AttnLossNoCrossModel, self).__init__()
        ...
        self.args = args
        info = f"""[model info] model: AttnLossNoCrossModel
                BERT model: {args.bert_model_path} 
                iterations: {args.iteration}
                pair_loss weight: {args.pair_weight}
                attn_loss weight: {args.attn_weight}
