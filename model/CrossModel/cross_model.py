...
class JointModule(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, bidirectional, layer_norm, lstm_share_param, cross_update, attention='tanh', output_dim=None, dropout=0.5):
    ...
    def forward(self, review_input, reply_input, table_input, review_input_mask, reply_input_mask, review_seq_lens, reply_seq_lens):
        ...
        if self.cross_update:
            ...
            return grid_feature, review_lstm_embedding, reply_lstm_embedding, attn_review, attn_reply
...
class CrossModel(nn.Module):
    ...
    def __init__(self, args, bertModel):
        ...
        self.encoder = nn.ModuleList([JointModule(args.bert_feature_dim, args.hidden_dim, args.bidirectional, args.layer_norm, args.lstm_share_param, args.cross_update, attention=args.attention, dropout=args.dropout) if i == 0 \
                                      else JointModule(args.hidden_dim, args.hidden_dim, args.bidirectional, args.layer_norm, args.lstm_share_param, args.cross_update, attention=args.attention, dropout=args.dropout) for i in range(args.iteration)])
        ...
        if args.cnn_classifier:
            ...
            classifier = [nn.Conv2d(100, 50, args.kernel_size, padding=args.kernel_size//2), nn.ReLU(), nn.Conv2d(50, 2, args.kernel_size, padding=args.kernel_size//2)]
        else:
            classifier = [nn.Linear(args.hidden_dim*2, 100), nn.ReLU(), nn.Linear(100, 50), nn.ReLU(), nn.Linear(50, 2)]
        ...
        info = f"""[Model INFO]
                ...
                pair loss weight: {args.pair_weight}
                attention loss weight: {args.attn_weight}
                BERT feature dimension: {args.bert_feature_dim} 
                hidden dimension: {args.hidden_dim}
                cross update: {args.cross_update}
                attention mechanism: {args.attention}
                attention loss: {args.attention_loss}"""
